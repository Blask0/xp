...
final class NetworkInterfaceResolver
{
    ...
    public String resolveAddress( final String addressOrInterfaceName )
    {
        ...
        final String[] interfaceParts = interfaceName.split( ":", 2 );

        if ( interfaceParts.length == 2 )
        {
            ifaceName = interfaceParts[0];
            String ipProto = interfaceParts[1];
        ...
        throws UnknownHostException, SocketException
    {
        ...
        if ( "local".equals( ifaceName ) )
        {
            if ( ipVersion == null )
            {
                // for backwards compatibility use ipv4 address.
                // In future version should use system preference via InetAddress.getLoopbackAddress().
